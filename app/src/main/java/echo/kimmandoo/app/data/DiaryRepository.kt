...
sealed class ReceiveLetterResult {
    data class Success(val diary: Diary, val paidWith: String) : ReceiveLetterResult()
    object NoDiaryAvailable : ReceiveLetterResult()
    object InsufficientFunds : ReceiveLetterResult()
    data class Error(val message: String) : ReceiveLetterResult()
}
...
class DiaryRepository(
    ...
    suspend fun getUserFreeChanceCount(): Long {
        ...
        return try {
            val document = db.collection(USERS_COLLECTION).document(currentUser.uid).get().await()
    ...
    suspend fun receiveLetter(useCurrency: Boolean): ReceiveLetterResult {
        ...
        val diaryToRead = findUnreadDiary(currentUser.uid)
            ?: return ReceiveLetterResult.NoDiaryAvailable

        val userRef = db.collection(USERS_COLLECTION).document(currentUser.uid)
        return try {
            val paidWith = db.runTransaction { transaction ->
                val userSnapshot = transaction.get(userRef)

                val paidResult = if (useCurrency) {
    ...
    suspend fun saveReply(
        ...
        val originalDiary = db.collection(DIARIES_COLLECTION).document(originalDiaryId).get().await().toObject(Diary::class.java)
        ...
        return try {
            db.runBatch {
                ...
                if (originalAuthorId != null) {
                    ...
                    val receivedActivity = ActivityItem(
    ...
    suspend fun getRepliesForDiary(diaryId: String): List<Reply> {
        return try {
            db.collection(REPLIES_COLLECTION)
    ...
    suspend fun getMyDiariesForMonth(year: Int, month: Int): List<Diary> {
        val currentUser = auth.currentUser ?: return emptyList()
        val startDate = Date.from(LocalDate.of(year, month, 1).atStartOfDay(java.time.ZoneId.systemDefault()).toInstant())
        val endDate = Date.from(LocalDate.of(year, month, 1).plusMonths(1).atStartOfDay(java.time.ZoneId.systemDefault()).toInstant())
